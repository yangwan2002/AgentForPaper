...
def test_prepare_claim_text_joins_pdf_wraps_and_removes_bibliography():
    content = (
        "该方法使用视觉标\n"
        "记完成定位[7]，并提升鲁棒性。\n"
        "参考文献(References):\n"
    ...
    prepared = prepare_claim_text(content)
    assert "视觉标记完成定位[7]" in prepared
    assert "Olson" not in prepared
    verified, unverified = extract_pairs("s", prepared, {"7"})
    assert unverified == []
    assert len(verified) == 1
    assert "并提升鲁棒性" in verified[0].claim_sentence


def test_optional_citation_scope_excludes_authors_downstream_claim():
    content = (
        "本文贡献包括：同时结合特定视觉特征AprilTags[7]"
        "通过融合惯性信息修正全局地图。"
    )
    verified, _ = extract_pairs(
        "s", content, {"7"}, scope_to_citation=True
    )
